...
== %IMOEN_BANTER% @5 // ~Your brother sounds like he's all puffy huffy.~
...
== %ALORA_BANTER% @14 // ~Oh, uh, thank you!
...
== %ALORA_BANTER% @17 // ~Whoa, how did you catch me?~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyBranwen1","GLOBAL",0)~ THEN %BRANWEN_BANTER% X3milyBranwen1
...
== %BRANWEN_BANTER% @21 // ~What a meek thing to say. You can help more at the front.~
== X3milyB @22 // ~Oh, a great idea. I'll just put away my best tool and utilize my non-talents instead.~
== %BRANWEN_BANTER% @23 // ~*Snort*. Suit yourself.~
EXIT
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyCoran1","GLOBAL",0)~ THEN %CORAN_BANTER% X3milyCoran1
...
== %CORAN_BANTER% @27 // ~I could give you a few pointers, when we next stop to rest for a bit.
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyDynaheir1","GLOBAL",0)~ THEN %DYNAHEIR_BANTER% X3milyDynaheir1
...
== %DYNAHEIR_BANTER% @31 // ~We are all on some rite of passage. The transition between girl and woman. I shall overcome, and thou shall too.~
== X3milyB @32 // ~Your vote of confidence is really appreciated, Dynaheir.~
== %DYNAHEIR_BANTER% @33 // ~I wish to see another fine woman succeed, and thou'rt one.~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyEdwin1","GLOBAL",0)~ THEN %EDWIN_BANTER% X3milyEdwin1
...
== %EDWIN_BANTER% @37 // ~If you find yourself cowering from the lack of fortified walls, or restless at night, my bedroll has room for two.~
...
== %ELDOTH_BANTER% @36 //~A lady should keep quiet, and speak only when spoken to.~
== X3milyB @37 // ~I can make my point without words, if you like. You'll regret it.~
== %ELDOTH_BANTER% @38 // ~You're too soft for that, little angel. Now stop your whining.~
EXIT
...
== %FALDORN_BANTER% @40 // ~Because they cannot, and have not. If you have not lived so long a sheltered life, you would see it.~
== X3milyB @41 // ~Teaching the right way goes a lot further than hurting people.~
== %FALDORN_BANTER% @42 // ~And what of justice for what was done, foolish girl?~
== X3milyB @43 // ~It need not be cruel or harsh. Positive reinforcement goes so much further.~
== %FALDORN_BANTER% @44 // ~You are naive, Emily. I'll speak no more with you.~
EXIT
...
== %JAHEIRA_BANTER% @53 // ~Yes? Where are you getting at with this?~
== X3milyB @54 // ~Well, was it awkward with your parents?~
== %JAHEIRA_BANTER% @55 // ~They did not live long in my childhood. I have only faint memories of that childhoom home in Tethyr.~
== X3milyB @56 // ~Oh. I didn't know. I'm sorry.~
== %JAHEIRA_BANTER% @57 // ~'Tis not your fault child, and it is the past. What matters is what is ahead.~
== X3milyB @58 // ~Will war ever end?~
== %JAHEIRA_BANTER% @59 // ~Not if we sit wondering.~
EXIT
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyKagain1","GLOBAL",0)~ THEN %KAGAIN_BANTER% X3milyKagain1
...
== %KAGAIN_BANTER% @62 // ~You're missing out, girl. Ammunition in this time is easy gold, and my store could use one.~
...
== %KHALID_BANTER% @65 // ~I g-get that a l-lot.~ 
== X3milyB @66 // ~But it's not true at all.~
== %KHALID_BANTER% @67 // ~R-really?~
...
== %KHALID_BANTER% @69 // ~I...T-thank you, Emily.~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyKivan1","GLOBAL",0)~ THEN %KIVAN_BANTER% X3milyKivan1
...
== %KIVAN_BANTER% @73 // ~There are many bows that would be suiting for your frame.~
...
== %MONTARON_BANTER% @377 // ~Shut it and keep your distance, goody archer. Your only use to the party is a packmule.~
== X3milyB @378 // ~That's a good way to speak to not make friends, Montaron.~
== %MONTARON_BANTER% @379 // ~Just leave me be, unless you want trouble.~
EXIT
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyQuayle1","GLOBAL",0)~ THEN %QUAYLE_BANTER% X3milyQuayle1
...
== %QUAYLE_BANTER% @82 // ~Well of course it is. If everyone was as smart as me, a tall task of course, progress would have people be less primitive.~
== X3milyB @83 // ~Not everyone has access to proper education. We first must strive to make things more fair.~
== %QUAYLE_BANTER% @84 // ~Yes, well...I suppose I do enjoy being vastly superior for the moment.~
...
== %SAFANA_BANTER% @87 // ~When you can get anything you want, what isn't to love about it?~
== X3milyB @88 // ~No one is fooled forever.~
== %SAFANA_BANTER% @89 // ~You've not seen me in action, darling. I could show you some of my tricks, you've the looks to pull it off.~
== X3milyB @90 // ~I'd rather create positive change in a more honest way.~
== %SAFANA_BANTER% @91 // ~When you see where that gets you, you know who to talk to.~
EXIT
...
CombatCounter(0)
!See([ENEMY])
Global("X3milySharTeel1","GLOBAL",0)~ THEN %SHARTEEL_BANTER% X3milySharteel1
...
== %SHARTEEL_BANTER% @94 // ~It's what makes life fun.~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milySkie1","GLOBAL",0)~ THEN %SKIE_BANTER% X3milySkie1
...
== %SKIE_BANTER% @98 // ~You worry too much, Emily. 
== X3milyB @99 // ~If he hurts you, in any way. Talk to me.~
== %ELDOTH_BANTER% IF ~InParty("Eldoth")~ THEN @100 // ~If you are talking about me, girl, you should do well to silence your lips.~
== X3milyB IF ~InParty("Eldoth")~ THEN @101 // ~I'll not shut up, Eldoth. Heed my warning, and treat her well.
== %SKIE_BANTER% IF ~InParty("Eldoth")~ THEN @102 // ~Emily, it's fine, really.~
== X3milyB IF ~InParty("Eldoth")~ THEN @103 // ~It isn't, but I will drop this for now.~
== %ELDOTH_BANTER% IF ~InParty("Eldoth")~ THEN @104 // ~As is your place to do, wench.~
EXIT
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyTiax1","GLOBAL",0)~ THEN %TIAX_BANTER% X3milyTiax1
...
== %TIAX_BANTER% @107 // ~Cyric shall bestow command unto Tiax for fulfilling his wishes. Tiax will be all powerful.~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyViconia1","GLOBAL",0)~ THEN %VICONIA_BANTER% X3milyViconia1
...
== %VICONIA_BANTER% @111 // ~Without <CHARNAME> you would be screaming away, child. Surfacers like you are so weak.~
== X3milyB @112 // ~Not listening!~
== %VICONIA_BANTER% @113 // ~So little spine.~
EXIT
...
== %XAN_BANTER% @115 // ~A world that we will one day no longer see. Everything is temporary, everything will cycle from joy to tragedy.~
== X3milyB @116 // ~You really know how to get someone down, don't you?~
== %XAN_BANTER% @117 // ~I am only speaking an age old truth, even if is hard to hear.~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyXzar1","GLOBAL",0)~ THEN %XZAR_BANTER% X3milyXzar1
...
== %XZAR_BANTER% @121 // ~Foolish harlot, the world is for those who grasp it. Destruction is the force of right.~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyYeslick1","GLOBAL",0)~ THEN %YESLICK_BANTER% X3milyYeslick1
...
== %YESLICK_BANTER% @125 // ~One day lass, I hope there won't be need for it all.~
== X3milyB @126 // ~Do you think of the people you have killed?~
== %YESLICK_BANTER% @127 // ~I have, but not as often as I think of those who I have lost because of them. My clan. My friends.~
== X3milyB @128 // ~I hope to find a way to change people before they hurt those who we love.~
== %YESLICK_BANTER% @129 // ~Idealistic lass. But perhaps you may learn such technique.~
EXIT
...
== %ALORA_BANTER% @138 // ~Oh, I'm just curious. Or it's shiny and interesting. I just have to look at it.~
...
== %ALORA_BANTER% @141 // ~Really? I accept!~
== X3milyB @142 // ~Great! This will be f- hey!~
== %ALORA_BANTER% @143 // ~Sorry, I just wanted to check out your bow.~
...
CombatCounter(0)
!See([ENEMY])
Global("X3milyDynaheir1","GLOBAL",1)~ THEN %DYNAHEIR_BANTER% X3milyDynaheir2
...
== %DYNAHEIR_BANTER% @155 // ~Thou'rt right, but we still have our present. Thy should enjoy the moments while it lasts.~
EXIT 
...
== %GARRICK_BANTER% @158 // ~Art, music, live well beyond our life, Emily. When we are but bone, they shall be spread like sweet honey.~
...
== %JAHEIRA_BANTER% @161 // ~What makes you say this, now?~
== X3milyB @162 // ~I've just been watching, and your presence makes me feel a little less nervous.~
== %JAHEIRA_BANTER% @163 // ~Ah I...thank you, Emily. Though pay attention to what is ahead so you may also remain alive.~
...
== %KHALID_BANTER% @166 // ~Oh, it w-was...oh, we shouldn't talk about this now. J-jaheria will get on us for attentiveness.~
== X3milyB @167 // ~Hrmph. Okay, but what about when we rest?~
== %KHALID_BANTER% @168 // ~I g-guess that would be alright.~
EXIT 
...
== %KIVAN_BANTER% @170 // ~She was a strong woman, who helped me become a better man.~
== %KIVAN_BANTER% @171 // ~I did not deserve her, or her love, but I am glad all the more that I had it.~
== %KIVAN_BANTER% @172 // ~I will never forgive those who took her from me.~
== X3milyB @173 // ~I will do all I can to make sure no one else ends up like your wife.~
== %KIVAN_BANTER% @174 // ~An unrealistic goal, Emily, but you are all the purer for it.~
EXIT
...
== %MINSC_BANTER% @176 // ~Me? Minsc feels alive in battle. It is evil that should be afraid!~
== X3milyB @177 // ~You fight in such a frenzy. You hurt yourself more when you lose focus.~
== %MINSC_BANTER% @178 // ~Minsc is very focused. Very focused on the butt-kicking!~
...
== %IMOEN_BANTER% @181 // ~Just a bit of fun. Everyone was a bit stuffed in books with their noses back in Candlekeep.~
== X3milyB @182 // ~So it's all just for a spot of mischief? I can't see the monks liking that.~
== %IMOEN_BANTER% @183 // ~Nah, but it wasn't anything I couldn't sweet talk them out of.~
...
== %ALORA_BANTER% @196 // ~Thanks, Emily. You were a great help.~ 
== %ALORA_BANTER% @197 // ~When is the next lesson?~
== X3milyB @198 // ~To be honest, Alora, your aim nearly matches mine already. I don't think I have anything to teach you.~
== %ALORA_BANTER% @199 // ~Why don't we improve eachother, then? It will be fun!~
...
== %CORAN_BANTER%  @202 // ~Oh? No, no, beauty. It was a lovely lesson but...~
== X3milyB @203 // ~Bow for life then, hmm? It is not a bad weapon.~
== %CORAN_BANTER% @204 // ~But still I enjoyed the lesson with true beauty guiding my hand.~
...
== %KHALID_BANTER% @210 // ~Perhaps so, Emily, but b-by your story it seems genuine.~
== X3milyB @211 // ~Maybe I'm feeling guilty. Still, thank you for sharing. I feel a real kinship with you.~
== %KHALID_BANTER% @212 // ~T-thank you. Your company is a w-welcome part here for me.~
EXIT 
...
DO ~SetGlobal("X3milyCoran1","GLOBAL",4)~
== %CORAN_BANTER% @214 // ~Anything you wish, angel.~
== X3milyB @215 // ~Do you actually care for Safana?~
== %CORAN_BANTER% @216 // ~Well, she's a self serving wench, but that just adds to her attractiveness.~
== X3milyB @217 // ~How do you find that desirable?~
== %CORAN_BANTER% @218 // ~Every piece of gorgeous art is imperfect to the eye, but the flaws only add to the character.~
...
!See([ENEMY])
Race("X3mily",Aasimar) 
Global("X3milyGarrick1","GLOBAL",2)~ THEN %GARRICK_BANTER% X3milyGarrick3
...
== %GARRICK_BANTER% @223 // ~Your voice has always been sweet, your aura calling and calming.~
== X3milyB @224 // ~I'm sorry, I can't take those words as genuine. I just can't. Just leave me alone for now.~ 
== %GARRICK_BANTER% @225 // ~As you wish.~
EXIT 